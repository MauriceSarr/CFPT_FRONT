    ...
    <dialog data-modal-ajout class="modal">
        <div class="cote_droit contenu">
            <form @submit.prevent="validerAvantAjout()" method="">
                <h1 class="sous_titre">Ajout Personnel Appui</h1>
...
export default {
  components: {
    ...
    name: "createPersonnelAppuiCompenent",
    data() {
        return {
            users: [],
            form: new Form({
    ...
    methods: {
        ...
        async soumettre() {
            ...
            try {
                const create_store = await axios.post('/personnel_appui/store', formdata);

                this.resetForm();
                bus.emit('personnelAppuiAjoutee');
