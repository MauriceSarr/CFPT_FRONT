        ...
        <div class="page-bar">
            <div class="page-title-breadcrumb">
                ...
                <ol class="breadcrumb page-breadcrumb pull-right">
                    ...
                    <li><a class="parent-item" :href="'/rayon/accueil'"> Rayon </a>&nbsp;<i
                            class="fa fa-angle-right"></i>
                    </li>
                </ol>
            </div>
            ...
            <div class="col-md-12">
                <div class="tabbable-line">
                    ...
                    <ul class="nav customtab nav-tabs" role="tablist">
                        <li class="nav-item"><a href="#tab1" class="nav-link active" data-bs-toggle="tab">Rayon</a>
                        </li>
                                        ...
                                        <div class="card-body ">
                                            <div class="row">
                                                <div class="col-md-6 col-sm-6 col-6">
                                                    <div class="btn-group">
                                                        <a :href="'/rayon/create'" id="addRow" class="btn btn-primary">
                                            ...
                                            <table
                                                ...
                                                <tbody>
                                                    <tr class="odd gradeX" v-for="(rayon, index) in rayons"
                                                        :key="index">
                                                        <td>{{ index + 1 }}</td>
                                                        ...
                                                        <td class="left">
                                                            <a class="tblEditBtn" @click="openModal(rayon)">
                                                                <i class="fa fa-pencil"></i>
                                                            </a>
                                                            <a class="tblDelBtn" @click="deleteRayon(rayon)">
    ...
    <div class="page-content-wrapper" v-show="editModal">
        <div class="page-content">
            <div class="page-bar">
                <div class="page-title-breadcrumb">
                    <div class=" pull-left">
                        <div class="page-title">Nouveau Rayon</div>
                    </div>
                    <ol class="breadcrumb page-breadcrumb pull-right">
                        ...
                        <li><a class="parent-item" href="{{ route('rayon_create') }}">Rayon</a>&nbsp;<i
                                class="fa fa-angle-right"></i>
                        </li>
                        <li class="active">Modifier Rayon</li>
                    </ol>
                </div>
            </div>
            <div class="row">
...
import FormulaireModification from './createRayonComponent.vue';
...
export default {
  components: {
    ...
    mounted() {
        this.get_rayon();
        bus.on('rayonAjoutee', () => { // Écouter l'événement de nouvelle utilisateur ajoutée
            this.get_rayon(); // Mettre à jour la liste des utilisateurs
        });
        bus.on('rayonDejaModifier', (eventData) => {
    ...
    methods: {
        ...
        async get_rayon() {
            ...
            await axios.get('/rayon/index', {headers})
                .then(response => {
                    const allrayon = response.data.rayon
                    const formattedRayon = allrayon.map(ray => {
                        return {
                            id: ray.id,
                            intitule: ray.intitule,
                            description:ray.description,
                            editModal: true,
                        };
                    });

                    this.rayons = formattedRayon;
                    this.initDataTable();


                }).catch(error => {
                    //Swal.fire('Erreur!', 'Une erreur est survenue lors de la recuperation des rayons', 'error')
                    showDialog3("Une erreur est survenue lors de la recuperation des rayons")
                });
        },
        ...
        async deleteRayon(type) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete(`/rayon/delete/${type.id}`, {headers}).then(resp => {
                        showDialog6("Rayon supprimé avec succés")
                        this.get_rayon();

                    }).catch(function (error) {
                        console.log(error);
                        showDialog3("Une erreur est survenue lors de la suppression du rayon")
        ...
        openModal(rayon) {
            ...
            bus.emit('rayonModifier', eventData);
        },
    }
}
</script>