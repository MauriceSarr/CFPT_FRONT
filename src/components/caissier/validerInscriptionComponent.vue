    ...
    methods: {
        ...
        async soumettre() {
         ...
         if (this.selectEleve.montant > 0) {
            ...
            try {
                const create_store = await axios.post('/caissier/valider-inscription/' + this.search_query, formdata, {headers});
        ...
        async get_classe() {
            const token = localStorage.getItem("token");
            const headers = { Authorization: `Bearer ${token}` };
            await axios.get('/classe/all', {headers}).then(response => {
        ...
        async update_inscription(id) {
            ...
            try {
                ...
                await axios.post('/inscription/update/' + id, formdata, {headers});
