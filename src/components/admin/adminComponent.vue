                ...
                <div class="card-body">
                  <div class="table-responsive">
                    <table class="table table-striped custom-table table-hover">
                      ...
                      <tbody>
                        <tr
                          v-for="(count, role) in countRoleOccurrences()"
                          :key="role"
                        >
                          <td>{{ count }}</td>
                          <td>{{ role }}</td>
                          <td>
                            <!-- Ajoutez ici le graphique ou ce que vous souhaitez afficher -->
                            <!-- Notez que le graphique doit être différent pour chaque utilisateur -->
                          </td>
                          <td>
                            <a href="javascript:void(0)" class="tblEditBtn">
                              <i class="fa fa-pencil"></i>
  ...
  /*   async get_formateur() {
    ...
    if (this.token) {
        ...
        try {
            ...
            const response = await axios.get("/formateur/index", { headers });
